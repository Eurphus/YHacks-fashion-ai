...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # res_x = x
        # # cloth adapter
        # cloth_token = self.img_mlp(cloth_token)
        # # print("x and cloth token size: ", x.size(), cloth_token.size())
        # shift, scale = self.adaLN_modulation(cloth_token).chunk(2, dim=2)
        # # print(self.norm_adapter(x).size(), shift.size(), scale.size())
        # x = res_x + modulate(self.norm_adapter(x), shift, scale)
        # # print("after adapter: ", x.size())
