    ...
    def __call__(
        ...
        if init_video is not None:
            if (mask_video == 255).all():
                ...
                # print(mask_condition.size())
                # masked_video = init_video * (1 - torch.tile(mask_condition, [1, 3, 1, 1, 1]))
                masked_video = self.image_processor.preprocess(rearrange(masked_video, "b c f h w -> (b f) c h w"), height=height, width=width) 
        ...
        # Prepare pose_video latent variables
        if pose_video is not None:
            ...
            pose_video_latents = self.prepare_control_latents(
                None,
                pose_video,
                batch_size,
                height,
            ...
            pose_latents = (
                torch.cat([pose_video_latents] * 2) if do_classifier_free_guidance else pose_video_latents
            ).to(device, weight_dtype)
        else:
            pose_video_latents = torch.zeros_like(latents).to(device, weight_dtype)
            pose_latents = (
                torch.cat([pose_video_latents] * 2) if do_classifier_free_guidance else pose_video_latents
            ).to(device, weight_dtype)

        # Prepare cloth image latent variables
        if cloth_image is not None:
            video_length = cloth_image.shape[2]
            cloth_image = self.image_processor.preprocess(rearrange(cloth_image, "b c f h w -> (b f) c h w"), height=height, width=width) 
            cloth_image = cloth_image.to(dtype=torch.float32)
            cloth_image = rearrange(cloth_image, "(b f) c h w -> b c f h w", f=video_length)
            cloth_image_latents = self.prepare_control_latents(
                None,
                cloth_image,
                batch_size,
                height,
            ...
            cloth_image_latents = torch.zeros_like(latents).to(device, weight_dtype)
            cloth_latents = (
                torch.cat([cloth_image_latents] * 2) if do_classifier_free_guidance else cloth_image_latents
            ).to(device, weight_dtype)
        
        if cloth_line_image is not None:
            video_length = cloth_line_image.shape[2]
            cloth_line_image = self.image_processor.preprocess(rearrange(cloth_line_image, "b c f h w -> (b f) c h w"), height=height, width=width) 
            cloth_line_image = cloth_line_image.to(dtype=torch.float32)
            cloth_line_image = rearrange(cloth_line_image, "(b f) c h w -> b c f h w", f=video_length)
            cloth_line_image_latents = self.prepare_control_latents(
                None,
                cloth_line_image,
                batch_size,
                height,
            ...
            cloth_line_latents = (
                torch.cat([cloth_line_image_latents] * 2) if do_classifier_free_guidance else cloth_line_image_latents
            ).to(device, weight_dtype)
        else:
            cloth_line_image_latents = torch.zeros_like(latents).to(device, weight_dtype)
            cloth_line_latents = (
                torch.cat([cloth_line_image_latents] * 2) if do_classifier_free_guidance else cloth_line_image_latents
            ).to(device, weight_dtype)

        ## get condition y ##
        y = [masked_video_latents_input, pose_latents, mask_input, cloth_latents, cloth_line_latents]
