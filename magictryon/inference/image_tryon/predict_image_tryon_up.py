...
############### load test data ##############
cloth_id_book = [
    "00345_00.jpg", "00641_00.jpg",  "00075_00.jpg", "00462_00.jpg", 
    "00739_00.jpg", "01430_00.jpg",  "04743_00.jpg",
    "01689_00.jpg", "01853_00.jpg",  "03032_00.jpg"
    ]

video_id = ["00055_00.jpg" , "00071_00.jpg", "00827_00.jpg", "01320_00.jpg", "01296_00.jpg"]


save_path = "samples/Image-Demo-up"
...
for cloth_id in cloth_id_book:
    # 获取衣服信息
    cloth_image_path        = os.path.join("datasets/garment/upper_body/cloth/", cloth_id)
    cloth_line_image_path   = os.path.join("datasets/garment/upper_body/cloth_anilines/", cloth_id)
    cloth_text = get_description_from_json(cloth_id, "datasets/garment/upper_body/caption_qwen.json")

    # 这里设置 prompt 与 negative_prompt
    ...
    for vid in video_id:
        mask_id = vid.replace('.jpg', '_mask.png')   

        org_video_path = os.path.join("datasets/person/vitonhd/image/", vid)
        masked_video_path = os.path.join("datasets/person/vitonhd/agnostic-v3.2/", vid)
        mask_video_path = os.path.join("datasets/person/vitonhd/agnostic-mask/", mask_id)
        pose_video_path = os.path.join("datasets/person/vitonhd/image-densepose/", vid)

        width, height, _, _ = get_video_properties(org_video_path)
        ...
        for idx, (pixel_value, masked_video_value, control_pixel_value, mask_value, cloth_pixel_value, cloth_line_pixel_value) in enumerate(zip(
            ...
            save_videos_grid(pixel_value, f"{save_path}/sanity_check/{video_name}/{video_name}_{cloth_name}_input.mp4", fps=fps, rescale=False)
            save_videos_grid(masked_video_value, f"{save_path}/sanity_check/{video_name}/{video_name}_{cloth_name}_masked_video.mp4", fps=fps, rescale=False)
            save_videos_grid(cloth_pixel_value, f"{save_path}/sanity_check/{video_name}/{video_name}_{cloth_name}_cloth.mp4", fps=fps, rescale=False)
        ...
        def save_results():
            ...
            if video_length == 1:
                video_path = os.path.join(save_path, "sanity_check",video_name, video_name+"_"+ cloth_name + ".png")
                image = results[0, :, 0].transpose(0, 1).transpose(1, 2)
                image = (image * 255).numpy().astype(np.uint8)
                Image.fromarray(image).save(video_path)
            else:
                video_path = os.path.join(save_path,"sanity_check",video_name, video_name+"_"+ cloth_name + ".mp4")
