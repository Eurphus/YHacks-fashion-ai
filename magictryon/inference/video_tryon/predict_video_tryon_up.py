...
subdir = "upper_body"
video_id = ["1219222_detail.mp4", "1233058_detail.mp4"]


save_path = "samples/Video-Demo-up"
use_repaint = False
no_all_frame = True
...
for cloth_id in cloth_id_book:
    ...
    for vid in video_id:
        org_video_path = os.path.join("datasets/person/ViViD", subdir, "videos", vid)
        masked_video_path  = os.path.join("datasets/person/ViViD", subdir, "agnostic", vid)
        mask_video_path = os.path.join("datasets/person/ViViD", subdir, "agnostic_mask", vid)
        pose_video_path = os.path.join("datasets/person/ViViD", subdir, "densepose", vid)
        ...
        os.makedirs(os.path.join(save_path, "sanity_check", subdir), exist_ok=True)
        ...
        for idx, (pixel_value, masked_video_value, control_pixel_value, mask_value, cloth_pixel_value, cloth_line_pixel_value) in enumerate(zip(
            ...
            save_videos_grid(pixel_value, f"{save_path}/sanity_check/{subdir}/{video_name}/{video_name}_{cloth_name}_input.mp4", fps=fps, rescale=False)
            save_videos_grid(masked_video_value, f"{save_path}/sanity_check/{subdir}/{video_name}/{video_name}_{cloth_name}_masked_video.mp4", fps=fps, rescale=False)
            save_videos_grid(cloth_pixel_value, f"{save_path}/sanity_check/{subdir}/{video_name}/{video_name}_{cloth_name}_cloth.mp4", fps=fps, rescale=False)
        ...
        def save_results():
            ...
            if video_length == 1:
                ...
                video_path = os.path.join(save_path,"sanity_check",subdir,video_name, video_name+"_"+ cloth_name + ".mp4")
